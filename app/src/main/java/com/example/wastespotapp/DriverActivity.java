...
public class DriverActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        markers.add(mMap1.addMarker(new MarkerOptions().position(driverLocation).title("driver")));
        markers.add(mMap1.addMarker(new MarkerOptions().position(requestLocation).title("Request").icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_BLUE))));
                ...
                if(e==null){
                    if(objects.size()>0){
                        for(ParseObject object:objects){
                            object.put("currentUsername", ParseUser.getCurrentUser().getUsername());
